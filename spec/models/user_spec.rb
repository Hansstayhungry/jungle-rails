require 'rails_helper'

RSpec.describe User, type: :model do
  describe 'Validations' do
    it "is valid" do
      user = User.new(
        first_name: 'fn',
        last_name: 'ln',
        email: 'bob@bob.com',
        password: '123456789',
        password_confirmation: '123456789'
      )
      expect(user).to be_valid
    end

    it "email is missing" do
      user = User.new(email: nil)
      expect(user).to be_invalid
      expect(user.errors[:email]).to include("can't be blank")
  
      user.email = 'bob@bob.com' # valid state
      user.valid?
      expect(user.errors[:email]).not_to include("can't be blank")
    end

    it "first name is missing" do
      user = User.new(first_name: nil)
      expect(user).to be_invalid
      expect(user.errors[:first_name]).to include("can't be blank")
  
      user.first_name = 'fn' # valid state
      user.valid? 
      expect(user.errors[:first_name]).not_to include("can't be blank")
    end

    it "last name is missing" do
      user = User.new(last_name: nil)
      expect(user).to be_invalid
      expect(user.errors[:last_name]).to include("can't be blank")
  
      user.last_name = 'ln' # valid state
      user.valid? 
      expect(user.errors[:last_name]).not_to include("can't be blank")
    end

    it "password don't match" do
      user = User.new(
        first_name: 'fn',
        last_name: 'ln',
        email: 'bob@bob.com',
        password: '123456789',
        password_confirmation: '12345678'
      )
      user.valid?
      expect(user.errors[:password_confirmation]).to be_present
    end

    it 'email must be unique' do
      user = User.new
      user.first_name = 'fn'
      user.last_name = 'ln'
      user.email = 'bob@bob.com'
      user.password = '123456789'
      user.password_confirmation = '123456789'

      user.save
    
      user2 = User.new
      user2.first_name = 'first_name'
      user2.last_name = 'last_name'
      user2.email = 'bob@bob.com'
      user2.password = '12345678'
      user2.password_confirmation = '12345678'
      user2.save
    
      expect(user2.errors[:email].first).to eq('has already been taken')
    end

    it 'password less than 7 characters is invalid' do
      user = User.new
      user.first_name = 'fn'
      user.last_name = 'ln'
      user.email = 'abc@abc.com'
      user.password = '123456'
      user.password_confirmation = '123456'
      expect(user).to be_invalid
    end

    it 'password length must be at-least 7 characters' do
      user = User.new
      user.first_name = 'fn'
      user.last_name = 'ln'
      user.email = 'abc@abc.com'
      user.password = '123456789'
      user.password_confirmation = '123456789'
      expect(user).to be_valid
    end
  end
  





  describe '.authenticate_with_credentials' do
    it 'should pass with valid credentials' do
      user = User.new(
        first_name: 'fn',
        last_name: 'ln',
        email: 'bob@bob.com',
        password: '123456789',
        password_confirmation: '123456789'
      )
      user.save

      user = User.authenticate_with_credentials('bob@bob.com', '123456789')
      expect(user).not_to be(nil)
    end

    it 'should not pass with invalid credentials' do
      user = User.new(
        first_name: 'fn',
        last_name: 'ln',
        email: 'bob2@bob.com',
        password: '123456789',
        password_confirmation: '123456789'
      )
      user.save

      user = User.authenticate_with_credentials('bob2@bob.com', '12345678')
      expect(user).to be(nil)
    end

    it 'should pass even with spaces present in email' do
      user = User.new(
        first_name: 'first_name',
        last_name: 'last_name',
        email: 'bob@bob.com',
        password: '123456789',
        password_confirmation: '123456789'
      )
      user.save

      user = User.authenticate_with_credentials('  bob@bob.com  ', '123456789')
      expect(user).not_to be(nil)
    end

    it 'should pass even with caps present in email' do
      user = User.new(
        first_name: 'first_name',
        last_name: 'last_name',
        email: 'bob@bob.com',
        password: '123456789',
        password_confirmation: '123456789'
      )
      user.save

      user = User.authenticate_with_credentials('boB@Bob.com', '123456789')
      expect(user).not_to be(nil)
    end
  end
end
